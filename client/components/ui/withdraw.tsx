...
import { fetchTokenBalance, createWithdrawTransaction } from "../../lib/token-utils";
...
const Withdraw: React.FC<WithdrawProps> = ({ setSelectedCommand }) => {
  ...
  useEffect(() => {
    const getBalance = async () => {
      ...
      try {
        // For withdraw, we need to check the iToken balance (interest-bearing token)
        const balance = await fetchTokenBalance(account, address, fromCoin.name, true);
  ...
  const handleCoinSelect = (coin: CryptoCoin) => {
    // Only allow supported tokens for Nostra withdrawals
    if (!SUPPORTED_TOKENS.includes(coin.name)) {
      setError(`Only ${SUPPORTED_TOKENS.join(" and ")} are supported for withdrawals`);
      return;
    }
  ...
  const prepareWithdrawTransaction = async () => {
    ...
    if (parseFloat(fromAmount) > parseFloat(tokenBalance)) {
      setError(`Insufficient balance. You have ${tokenBalance} i${fromCoin.name}`);
      return;
    }
    ...
    try {
      // Create Nostra withdraw transactions using utility function
      const withdrawTransactions = createWithdrawTransaction(
        fromCoin.name,
        fromAmount,
        address
      );
      ...
      console.error("Error preparing withdraw transaction:", err);
      setError(err.message || "Failed to prepare withdraw transaction");
    } finally {
        ...
        <div className="flex items-start">
          ...
          <div className={`text-center flex-1`}>
            ...
            <p className={`text-lg font-bold text-black`}>
              {isLoadingBalance ? "Loading..." : `${tokenBalance} i${fromCoin.name}`}
            </p>
          </div>
        </div>
        ...
        <div className={`mt-5`}>
          {!txHash && !transactions.length && (
            <button 
              ...
              onClick={prepareWithdrawTransaction}
              disabled={isPreparing || !address || isLoadingBalance}
            >
              {isPreparing ? "Preparing..." : "Prepare Withdrawal"}
            </button>
          )}
          ...
          {transactions.length > 0 && !txHash && (
            <TransactionHandler
              ...
              description={`Withdraw ${fromAmount} ${fromCoin.name} from Nostra`}
