...
const Withdraw: React.FC<WithdrawProps> = ({ 
  ...
  const handleWithdraw = async () => {
    if (setShowWithdrawModal) setShowWithdrawModal(false);
    ...
    if (!address) {
      if (setMessages) {
        const errorMessage: Message = {
          id: uuidv4(),
          role: "agent",
          content: "Please connect your wallet first to proceed with the withdrawal.",
    ...
    const withdrawMessage = `Withdraw ${fromAmount} ${fromCoin.name}`;
      ...
      try {
        ...
        if (
          ...
          agentMessage = {
            ...
            role: "agent",
            content:
              "I'm sorry, I couldn't process your withdrawal request. Please try again with a different amount or token.",
            timestamp: new Date().toLocaleTimeString(),
            user: "Agent",
          };
        }
        ...
        const errorMessage: Message = {
          id: uuidv4(),
          role: "agent",
          content: "Sorry, something went wrong with your withdrawal. Please try again.",
          timestamp: new Date().toLocaleTimeString(),
          user: "Agent",
        };
        setMessages((prev) => [...prev, errorMessage]);
    ...
    <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-80 flex flex-col items-center justify-center animated fadeIn">
      <div className="bg-[#1A1A1A] p-6 w-[36rem] shadow-lg rounded-xl animated fadeIn">
        ...
        <div className="mt-6 grid grid-cols-2 gap-4">
          ...
          >
            <span>{isLoading ? <Loader className="animate-spin" color="white" size={16} /> : "Confirm Withdrawal"}</span>
