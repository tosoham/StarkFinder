...
export default function BlockNode({ data, isDragging, id }: BlockNodeInterface) {
  ...
  if (data.id === 'swap') {
    return <SwapNode data={{...data, isSelected}} isConnectable={true} id={''} selected={isSelected} type={''} zIndex={isSelected ? 50 : 0} xPos={0} yPos={0} dragging={false} />;
  }
  if (data.id === 'stake') {
    return <StakeNode data={{...data, isSelected}} isConnectable={true} type={''} id={''} selected={isSelected} zIndex={isSelected ? 50 : 0} xPos={0} yPos={0} dragging={false} />;
  }
  if (data.id === 'liquidity') {
    return <LiquidityNode data={{...data, isSelected}} isConnectable={true} id={''} selected={isSelected} type={''} zIndex={isSelected ? 50 : 0} xPos={0} yPos={0} dragging={false} />;
  }
  if (data.id === 'allocate') {
    return <AllocateNode data={{...data, isSelected}} isConnectable={true} selected={isSelected} type={''} zIndex={isSelected ? 50 : 0} xPos={0} yPos={0} dragging={false} id={''} />;
  }
  if (data.id === 'event') {
    return <EventNode data={{...data, isSelected}} isConnectable={true} id={''} selected={isSelected} type={''} zIndex={isSelected ? 50 : 0} xPos={0} yPos={0} dragging={false} />;
  }
