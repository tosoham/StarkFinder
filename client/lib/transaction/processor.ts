...
import { SwapHandler, TransferHandler, NostraDepositHandler, NostraWithdrawHandler, BridgeHandler, type BaseTransactionHandler } from "./handlers";
...
export class TransactionProcessor {
	...
	createTransactionData(response: BrianResponse): BrianTransactionData {
		...
		if (["swap"].includes(action) && response.extractedParams) {
			return {
	...
	formatTokenAmount = (rawAmount: number, decimals: number) => {
		if (!rawAmount || !decimals) return "0";

		// Convert to string if it's not already
		const amountStr = rawAmount.toString();

		// If the amount is shorter than decimals, we need to pad with zeros
		if (amountStr.length <= decimals) {
			const padding = "0".repeat(decimals - amountStr.length + 1);
			return `0.${padding}${amountStr}`.replace(/\.?0+$/, "");
		}

		// Insert decimal point at the right position
		const decimalPosition = amountStr.length - decimals;
		const wholeAmount = amountStr.substring(0, decimalPosition);
		const fractionalAmount = amountStr.substring(decimalPosition);

		// Remove trailing zeros
		const formattedFraction = fractionalAmount.replace(/0+$/, "");

		return formattedFraction ? `${wholeAmount}.${formattedFraction}` : wholeAmount;
	};
	...
	async processTransaction(response: BrianResponse): Promise<ProcessedTransaction> {
		try {
			...
			const toAmount = transactionData.toAmount || 0;

			const res = this.formatTokenAmount(Number(toAmount), response.data.toToken?.decimals || 0);
