...
async function processTransactionRequest(messageObj: Message, prompt: string): Promise<AxiosResponse> {
  try {
    ...
    const processedTx = await transactionProcessor.processTransaction(data.result[0]);
...
const commandHandlers: Record<string, {
  ...
  start: {
    execute: async (messageObj) => 
      sendMessage(messageObj, `Welcome to StarkFinder! 🚀...`), // Your existing welcome message
    requiresInput: false
  },
...
// Message Handler
async function handleMessage(messageObj: Message): Promise<AxiosResponse | undefined> {
  try {
    if (!messageObj?.from?.id) throw new Error('Invalid message object');
